...
export function VoidThemeToggle() {
  ...
  const toggleTheme = () => {
    ...
    const nextTheme: Theme = theme === 'dark' ? 'light' : theme === 'light' ? 'auto' : 'dark'
    
    // Get effective theme for immediate application
    const { getEffectiveTheme } = require('@/lib/themes')
    const effectiveTheme = getEffectiveTheme(nextTheme)
    
    // Instant switch - 0ms perceived delay
    document.documentElement.setAttribute('data-theme', effectiveTheme)
    document.documentElement.setAttribute('data-theme-preference', nextTheme)
    ...
    if (nextTheme === 'auto' && typeof window !== 'undefined') {
