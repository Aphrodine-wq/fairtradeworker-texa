...
const BreadcrumbComponent = ({ items, onNavigate, className }: BreadcrumbProps) => {
  ...
  return (
    <nav 
      ...
      className={cn(
        "flex items-center gap-2 text-sm px-4 py-3 bg-muted/30 backdrop-blur-sm rounded-lg border border-border/50",
        className
      )}
    >
            ...
            <li key={index} className="flex items-center gap-2">
              ...
              {isClickable ? (
                <motion.button
                  ...
                  whileTap={{ scale: 0.98 }}
                  className="flex items-center gap-1.5 px-2 py-1 rounded-md hover:bg-muted/60 transition-colors text-muted-foreground hover:text-foreground font-medium group"
