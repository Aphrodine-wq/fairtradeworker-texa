...
const categoryIcons: Record<MilestoneExpense['category'], any> = {
  materials: Package,
  labor: HardHat,
  equipment: Wrench,
  permits: FileText,
...
export function ExpenseTracking({ milestone, onUpdateMilestone, canEdit }: ExpenseTrackingProps) {
  ...
  const budgetRemaining = milestone.amount - totalExpenses
  const budgetPercentUsed = (totalExpenses / milestone.amount) * 100

  const expensesByCategory = expenses.reduce((acc, exp) => {
    acc[exp.category] = (acc[exp.category] || 0) + exp.amount
    return acc
  }, {} as Record<MilestoneExpense['category'], number>)
  ...
  const handleFileUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onloadend = () => {
      ...
      setFormData(prev => ({ ...prev, receiptPhoto: reader.result as string }))
    }
  ...
  const updateUnitCost = (cost: number) => {
          ...
          <div className="space-y-6">
            <div className="grid grid-cols-3 gap-4">
              ...
              <Card>
                <CardContent className="pt-6">
                  ...
                  <div className={`text-2xl font-bold ${budgetRemaining < 0 ? 'text-red-600' : 'text-green-600'}`}>
              ...
              <Card>
                <CardContent className="pt-6">
                  ...
                  <div className={`text-2xl font-bold ${budgetPercentUsed > 100 ? 'text-red-600' : ''}`}>
            ...
            {Object.keys(expensesByCategory).length > 0 && (
              <div>
                ...
                <div className="grid grid-cols-2 gap-3">
                  {(Object.entries(expensesByCategory) as [MilestoneExpense['category'], number][]).map(([category, amount]) => {
                    ...
                    return (
                      <div key={category} className="flex items-center gap-3 p-3 border rounded-lg">
                        ...
                        <div className="text-sm font-bold">${amount.toLocaleString('en-US', { minimumFractionDigits: 2 })}</div>
                      </div>
                    )
                  })}
                </div>
            ...
            {expenses.length > 0 ? (
                      ...
                      {expenses.map((expense) => {
                        ...
                        return (
                          <TableRow key={expense.id}>
                            ...
                            <TableCell>
                              <Badge className={`${categoryColors[expense.category]} border`}>
                                <Icon size={14} weight="bold" className="mr-1" />
                                {expense.category}
                              </Badge>
                            </TableCell>
                            <TableCell>
                            ...
                            <TableCell className="text-right">
                              {expense.quantity ? expense.quantity.toLocaleString() : '—'}
                            </TableCell>
                            <TableCell className="text-right">
                              {expense.unitCost ? `$${expense.unitCost.toLocaleString('en-US', { minimumFractionDigits: 2 })}` : '—'}
                            </TableCell>
              ...
              <div className="text-center py-8 text-muted-foreground">
                <Receipt size={48} weight="duotone" className="mx-auto mb-3 opacity-30" />
                <p>No expenses tracked yet</p>
                {canEdit && (
                  <p className="text-sm mt-2">Click "Add Expense" to start tracking costs</p>
                )}
              </div>
            )}
          </div>
