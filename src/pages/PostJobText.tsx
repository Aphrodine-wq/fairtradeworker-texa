...
import { Notebook, Plus, Trash, Camera, VideoCamera, Microphone, ArrowLeft } from "@phosphor-icons/react"
...
interface PostJobTextProps {
  user: User
  onNavigate: (page: string) => void
}

export function PostJobText({ user, onNavigate }: PostJobTextProps) {
        ...
        >
          <div className="text-center mb-8">
            <div className="inline-flex items-center justify-center w-16 h-16 rounded-full bg-primary/10 mb-4">
              <Notebook className="w-8 h-8 text-primary" weight="fill" />
            </div>
            <h1 className="text-3xl font-bold">Post Job with Text</h1>
            <p className="text-muted-foreground mt-2">Describe your job in detail and add media</p>
          </div>
          ...
          <Card className="border-2 border-primary/20">
            <CardHeader>
              <CardTitle className="flex items-center gap-2">
                <Notebook className="w-5 h-5 text-primary" weight="fill" />
                Job Description (Required)
              </CardTitle>
              <CardDescription>Provide detailed information about the work needed</CardDescription>
            </CardHeader>
            <CardContent>
              <Textarea
                placeholder="Describe the work you need done in detail. Include:&#10;- What's the problem?&#10;- Where is it located?&#10;- Any relevant history or context&#10;- Your timeline preferences"
                value={description}
          ...
          <div className="flex gap-4">
            ...
            <Button
              ...
              disabled={isProcessing || !title.trim() || description.trim().length < 20}
              className="flex-1"
            >
